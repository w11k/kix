...
object ChatServer extends LiftActor with ListenerManager  {
  ...
  override def lowPriority = {
    case ChatMsg(name, msg) if msg.trim.length > 0 =>
      chatLines ::= ChatLine(name, now, msg)
      chatLines = chatLines take 50
      updateListeners()
  }

  override protected def createUpdate = ChatServerUpdate(chatLines take 3)

  private var chatLines = List(ChatLine("kix", now, ?("Welcome")))
}
